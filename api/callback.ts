...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (tokenData.error) {
      ...
      return res
        .status(400)
        .json({ error: tokenData.error_description || 'OAuth failed' });
    }
    ...
    res.setHeader(
      'Set-Cookie',
      serialize('github_token', access_token, cookieOptions)
    );
    ...
    console.log(
      'Raw redirect_after_login cookie:',
      req.cookies.redirect_after_login
    );
    try {
